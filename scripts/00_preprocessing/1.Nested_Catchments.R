...
# Set working directory to project root and load paths
...
outhybas <- outletopen(hydro_dir, outletname)
outhybas$latlong <- paste(round(outhybas$Var1, 4), round(outhybas$Var2, 4), sep = " ")

## 2-a  LDD raster (NetCDF)  -------------------------------------------------
nc_path <- file.path(hydro_dir, "mapscal", "ldd_European_01min.nc")
ldd_raw <- rast(nc_path)


# Hybas07
Catchmentrivers7 <- read.csv(file.path(hydro_dir, "Catchments", "from_hybas_eu_onlyid.csv"), encoding = "UTF-8", header = T, stringsAsFactors = F)
hybas07 <- read_sf(dsn = file.path(hydro_dir, "Catchments", "hydrosheds", "hybas_eu_lev07_v1c.shp"))
...
outll <- outletopen(hydro_dir, outletname)
...
outhybas$latlong <- paste(round(outhybas$Var1, 4), round(outhybas$Var2, 4), sep = " ")
UpArea <- UpAopen(hydro_dir, outletname, outhybas)
head(UpArea)
...
for (i in 1:n) {
  ...
  inter_areas <- suppressMessages(vapply(valid_children, function(j) {
    ...
    if (is.null(overlap) || length(overlap) == 0 || st_is_empty(overlap)) {
      return(0)
    }
...
cat(
  "Outlet 290666 intersects polygon 292302:",
...
cat(sprintf(
  "Geometric overlap: %.1f km2 (%.1f%% of 290666)\n",
...
plot_nested_branch <- function(sf_data, matrix, row_index) {
  ...
  ggplot() +
    ...
    theme_minimal() +
    labs(
      title = paste("Catchment ID:", sf_data$catch_id[row_index]),
  ...
  if (length(immediate_child_indices) == 0) {
    ...
    if (new_row$residual_area_km2 > new_row$area_km2 + 1) {
      ...
      ggplot() +
        ...
        theme_minimal() +
        labs(
          title = "Inter-catchment (Residual) Area",
          subtitle = "Green represents area draining directly to the reach between stations"
        )
    }
...
final_residuals$nesting_count <- sapply(
  strsplit(as.character(final_residuals$all_nested_ids), ","),
  function(x) {
    if (all(is.na(x))) {
      return(0)
    }
