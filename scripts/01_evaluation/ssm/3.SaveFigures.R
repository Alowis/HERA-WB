...
#   Fig6  Aggregation-gain heatmap (Δrho vs daily)
...
strip_id <- function(df) { df$join_id <- as.character(gsub("^X","",df$catch_id)); df }
...
eu_mask <- st_read("D:\\gomezdi\\My Data\\0. Projects\\3.WildFireVulnerab\\Datasets\\TRACE\\Luc_delivery\\NUTS3_wgs84LatLon.shp",
                   quiet = TRUE)
EU30 <- c("HR","DE","BE","AT","ES","BG","FR","CZ","HU","EL","FI","CY","DK","EE",
...
message("  elev_m range: ", round(min(shp$elev_m, na.rm=TRUE),1),
...
message("all_meta rows: ", nrow(all_meta),
...
theme_nature <- function(base_size = 9) {
  theme_bw(base_size = base_size) %+replace%
    theme(
      ...
      plot.caption      = element_text(size = base_size - 1.5, colour = "grey50",
                                       hjust = 0, face = "italic"),
...
fig1_ann <- all_meta %>%
  group_by(scenario) %>%
  summarise(med = round(median(rho, na.rm = TRUE), 2),
...
fig1 <- ggplot(all_meta, aes(x = scenario, y = rho, fill = scenario)) +
  ...
  geom_boxplot(width = 0.12, outlier.shape = NA, linewidth = 0.45,
               colour = "grey25", fill = "white", fatten = 2) +
  geom_text(data = fig1_ann,
  ...
  labs(
    ...
    subtitle = paste0("Spearman \u03c1 across all European catchments (n = ",
...
fig2 <- ggplot(plot_dt_sub, aes(x = sm_lisf, y = sm_esa, colour = clim_class)) +
  ...
  geom_text(data = ann_overall,
            aes(x = ax_lim[1] + diff(ax_lim) * 0.04,
                y = ax_lim[2] - diff(ax_lim) * 0.06,
                label = paste0("\u03c1 = ", rho_all)),
            hjust = 0, vjust = 1, size = 2.5, colour = "grey15",
            fontface = "bold", inherit.aes = FALSE) +
  scale_colour_manual(values = clim_pal, name = "Climate class",
...
make_strat_box <- function(data, x_var, fill_var, fill_pal, fill_name,
  ...
  p <- ggplot(data, aes(x = .data[[x_var]], y = rho, fill = .data[[fill_var]])) +
    ...
    geom_boxplot(position = position_dodge(width = 0.8), width = 0.68,
...
p4a <- make_strat_box(filter(all_meta, !is.na(clim_class)),
...
compute_gain <- function(data, group_var, group_label) {
  ...
  long %>%
    ...
    mutate(delta_rho    = med_rho - baseline,
...
fig6 <- ggplot(gain_dt, aes(x = aggregation, y = stratum, fill = delta_rho)) +
  ...
  geom_text(
    aes(label  = sprintf("%+.2f", delta_rho),
...
write.csv(summarise_rho(all_meta, "clim_class"),
          file.path(path_out, "Table_rho_by_climate.csv"),   row.names = FALSE)
write.csv(summarise_rho(all_meta, "area_class"),
          file.path(path_out, "Table_rho_by_area.csv"),      row.names = FALSE)
write.csv(summarise_rho(all_meta, "elev_class"),
          file.path(path_out, "Table_rho_by_elevation.csv"), row.names = FALSE)
